...
class CustomNodeWrapper extends node_1.Node {
    ...
    constructor({ id, process, executionConfigPtr, }) {
        ...
        const callback = (executionId) => {
            ...
            // substract 1 because of the context
            const processInputsLength = process.length - 1;
            if (processInputsLength !== inputs.length) {
                throw new external_1.ExternalProcessError(`Invalid number of inputs for custom node ${id}, expected ${processInputsLength}, got ${inputs.length}`, external_1.ExternalProcessStatusCode.InvalidArgument);
            }
            ...
            const result = deasync.awaitSync(process.call(this, fakeContext, ...inputs));
