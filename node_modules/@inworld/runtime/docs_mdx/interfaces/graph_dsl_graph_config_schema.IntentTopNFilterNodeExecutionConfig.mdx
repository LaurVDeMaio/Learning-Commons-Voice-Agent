...
# Interface IntentTopNFilterNodeExecutionConfig

Configuration for executing intent top N filter nodes

This interface was referenced by `InworldBasicConfigurationSchema`'s JSON-Schema via the `definition` "IntentTopNFilterNodeExecutionConfig".

interface IntentTopNFilterNodeExecutionConfig {  
report_to_client?: string | boolean;  
top_n_intents?: string | number;  
}
...
report_to_client? top_n_intents?
