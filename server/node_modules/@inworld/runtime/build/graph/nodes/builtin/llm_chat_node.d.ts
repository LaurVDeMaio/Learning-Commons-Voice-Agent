import { Node, NodePropsInterface } from '../node';
import { LLMChatNodeExecutionConfigPropsInterface } from './node_execution_config';
/**
 * Interface for LLM Node creation properties.
 */
export interface LLMNodePropsInterface extends NodePropsInterface {
    /** Execution configuration for the LLM node */
    executionConfig: LLMChatNodeExecutionConfigPropsInterface;
    /** External node reference (for component registry approach) */
    external?: any;
}
/**
 * Node for language model text generation in the graph.
 * Processes input through a language model to generate text responses.
 */
export declare class LLMChatNode extends Node {
    private executionConfig;
    private stream;
    /**
     * Creates a new LLMChatNode instance.
     *
     * @param props - LLM chat node creation properties
     * @internal
     */
    constructor(props: LLMNodePropsInterface);
    /**
     * Cleans up resources associated with this LLM node.
     */
    destroy(): void;
}
