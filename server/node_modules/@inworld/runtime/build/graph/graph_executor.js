...
class GraphExecutor {
     ...
     * @internal
     */
    constructor(compiledGraph, executorConfig) {
        this.external = null;
        if (!compiledGraph) {
    ...
    async execute(data, executionId, userContext) {
        if (userContext) {
            return (await expose_binary_1.GraphExecutorFunctions.executeWithUserContext(this.external, data.getExternal(), executionId, userContext.getExternal()));
        }
        else {
            return (await expose_binary_1.GraphExecutorFunctions.execute(this.external, data.getExternal(), executionId));
        }
    }
    /**
    ...
    destroy() {
        if (this.external) {
            expose_binary_1.GraphExecutorFunctions.factoryHelperDelete(this.external);
            this.external = null;
        }
    }
}
