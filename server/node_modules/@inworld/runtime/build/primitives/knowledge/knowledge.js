...
class Knowledge {
     ...
     */
    async getKnowledge(knowledgeGetInput) {
        var _a, _b;
        const idsVector = new string_1.VectorString(knowledgeGetInput.ids);
        const history = knowledgeGetInput.eventHistory;
        try {
            const result = await expose_binary_1.KnowledgeInterfaceFunctions.getKnowledge(this.external, idsVector.getExternal(), history.getExternal());
            if (!expose_binary_1.KnowledgeVectorStatusFunctions.isOK(result)) {
                ...
                throw new inworld_1.InworldError('Failed to get knowledge: ' + ((_a = (0, helpers_1.getStatus)(status)) === null || _a === void 0 ? void 0 : _a.status), (0, helpers_1.getStatus)(status));
            }
            return string_1.VectorString.toArray(expose_binary_1.KnowledgeVectorStatusFunctions.getValue(result));
        }
        catch (error) {
            throw new inworld_1.InworldError('Failed to get knowledge: ' + ((_b = error === null || error === void 0 ? void 0 : error.context) === null || _b === void 0 ? void 0 : _b.status));
        }
        finally {
            idsVector.destroy();
        }
     ...
     */
    async compileKnowledge(knowledgeId, recordsOrFile) {
    ...
    async removeKnowledge(knowledgeId) {
        const result = await expose_binary_1.KnowledgeInterfaceFunctions.removeKnowledge(this.external, knowledgeId);
        if (!expose_binary_1.KnowledgeVectorStatusFunctions.isOK(result)) {
            const status = expose_binary_1.KnowledgeVectorStatusFunctions.getStatus(result);
            throw new inworld_1.InworldError('Failed to remove knowledge', (0, helpers_1.getStatus)(status));
        }
     ...
     * @throws InworldError If compilation fails
     */
    async compileKnowledgeFromRecords(knowledgeId, records) {
        // Create vector of records
        const recordsVector = new string_1.VectorString(records);
        try {
            const result = await expose_binary_1.KnowledgeInterfaceFunctions.compileKnowledgeFromRecords(this.external, knowledgeId, recordsVector.getExternal());
            if (!expose_binary_1.KnowledgeVectorStatusFunctions.isOK(result)) {
                const status = expose_binary_1.KnowledgeVectorStatusFunctions.getStatus(result);
                throw new inworld_1.InworldError('Failed to compile knowledge from records', (0, helpers_1.getStatus)(status));
            }
     ...
     * @throws InworldError If compilation fails
     */
    async compileKnowledgeFromFile(knowledgeId, file) {
        var _a;
        let bytesVector;
        try {
            const buffer = Buffer.from(file.content.bytes);
            bytesVector = expose_binary_1.VectorCharFunctions.memcpyTo(buffer, buffer.length);
            const result = await expose_binary_1.KnowledgeInterfaceFunctions.compileKnowledgeFromFile(this.external, knowledgeId, bytesVector, this.convertFileTypeToNumber(file.type));
            if (!expose_binary_1.KnowledgeVectorStatusFunctions.isOK(result)) {
                const status = expose_binary_1.KnowledgeVectorStatusFunctions.getStatus(result);
                ...
                throw new inworld_1.InworldError(`Failed to compile knowledge from file: ${statusStr}`, (0, helpers_1.getStatus)(status));
            }
            return string_1.VectorString.toArray(expose_binary_1.KnowledgeVectorStatusFunctions.getValue(result));
        }
        catch (error) {
            throw new inworld_1.InworldError(`Failed to compile knowledge from file: ${error.message}`);
        }
        finally {
    ...
    convertFileTypeToNumber(fileType) {
        switch (fileType) {
            ...
            default:
                throw new inworld_1.InworldError(`Unknown file type: ${fileType}`);
        }
    }
    /**
