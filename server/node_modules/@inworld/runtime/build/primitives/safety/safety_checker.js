...
class SafetyChecker {
    ...
    static async create(embedder, config) {
        if (!embedder) {
            throw new inworld_1.InworldError(`Error creating SafetyChecker: Embedder is required`);
        }
        if (!config) {
            throw new inworld_1.InworldError(`Error creating SafetyChecker: Config is required`);
        }
        if (!config.modelWeightsPath) {
            throw new inworld_1.InworldError(`Error creating SafetyChecker: Model weights path is required`);
        }
        const instance = new SafetyChecker(embedder);
        try {
            instance.nativeSafetyChecker =
                await instance.createNativeSafetyChecker(config);
            return instance;
        }
        catch (err) {
            instance.destroy();
            if (err instanceof inworld_1.InworldError) {
                throw err;
            }
            throw new inworld_1.InworldError(`Error initializing SafetyChecker: ${err.message}`, { cause: err });
        }
    }
    /**
     * @internal
     ...
     */
    async createNativeSafetyChecker(config) {
        let factory;
        let creationConfig;
        let result;
        try {
            ...
            result = await expose_binary_1.SafetyCheckerFactoryFunctions.createSafetyChecker(factory, this.embedder.getExternal(), creationConfig);
            if (!expose_binary_1.StatusOrSafetyCheckerInterfaceFunctions.isOK(result)) {
                ...
                throw new inworld_1.InworldError(`Error creating safety checker: ${statusContext}`);
            }
     ...
     */
    async checkSafety(input, config) {
        try {
            if (!this.nativeSafetyChecker) {
                throw new inworld_1.InworldError(`Error checking safety: Safety checker not initialized`);
            }
            if (!input || typeof input.input !== 'string') {
                throw new inworld_1.InworldError(`Error checking safety: Input must contain a non-empty string`);
            }
            let nativeConfig;
            let result;
            let response;
            try {
                ...
                result = nativeConfig
                    ? await expose_binary_1.SafetyCheckerInterfaceFunctions.checkSafetyWithConfig(this.nativeSafetyChecker, input.input, nativeConfig)
                    : await expose_binary_1.SafetyCheckerInterfaceFunctions.checkSafety(this.nativeSafetyChecker, input.input);
                ...
                if (!isOk) {
                    ...
                    throw new inworld_1.InworldError(`Error checking safety: ${statusResult}`);
                }
            ...
            throw new inworld_1.InworldError(`Error checking safety: ${err instanceof Error ? err.message : String(err)}`, {
                cause: err,
            });
        }
    }
        ...
        try {
            ...
            if (detectedTopics) {
                try {
                    ...
                    throw new inworld_1.InworldError(`Error processing detected topics: ${err.message}`, { cause: err });
                }
            }
            return {
            ...
            throw new inworld_1.InworldError(`Error converting safety response: ${err.message}`, {
                cause: err,
            });
        }
    }
    ...
    destroy() {
        ...
        if (this.embedder && typeof this.embedder.destroy === 'function') {
            try {
                ...
                throw new inworld_1.InworldError(`Error destroying SafetyChecker: Error during embedder cleanup: ${err.message}`, { cause: err });
            }
            finally {
                this.embedder = null;
            }
